...
import { Const, isDesktop, SECOND_IN_MILLIS } from "@tutao/app-env"
...
export class DriveViewModel {
	...
	constructor(
		...
		this.driveOperations.setAllTransfersDoneListener(() => {
	...
	cut(items: readonly FolderItem[]) {
		this.driveOperations.cut(items)
	...
	async paste() {
		...
		this.driveOperations.paste(this.currentFolder.folder)
		this.updateUi()
	}
	...
	async moveToTrash(items: readonly FolderItemId[]) {
		...
		await this.driveOperations.moveToTrash(items)
		this.listModel.selectNone()
	}
	...
	async downloadFile(file: DriveFile): Promise<void> {
		this.ensureWindowCloseListener()
		await this.driveOperations.downloadFile(file)
	}
	...
	flushTransfers() {
		this.driveOperations.flushTransfers()
	}
