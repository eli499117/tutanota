...
export class DriveModel {
	...
	async paste(currentFolder: DriveFolder) {
		...
		for (const file of files) {
			...
			if (choice === "cancel") {
				...
				if (choice === "keepBoth") {
					...
					const itemToReplace = assertNotNull(folderItems.files.findLast((item) => item.name === fileName))
					await this.moveToTrash([{ type: "file", id: itemToReplace._id }])
				}
