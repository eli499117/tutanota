...
import { assertNonNull, assertNotNull, incrementMonth, isNotEmpty, lastIndex, lazyAsync, stringToBase64, tokenize } from "../../../../platform-kit/utils"
...
import { encodeCalendarSearchKey } from "../../calendar/search/model/CalendarSearchUtils"
...
export class CalendarSearchModel {
	...
	async searchCalendar(searchQuery: SearchQuery, abortSignal: AbortSignal): Promise<LiveSearchResult<CalendarEvent>> {
		...
		const entityUpdatesListener: EntityUpdatesListener = {
			id: `Calendar LiveSearchResult ${searchQuery.query}`,
	...
	async runCalendarSearch(searchQuery: SearchQuery, abortSignal: AbortSignal): Promise<{ tokens: string[]; resultItems: CalendarEvent[] }> {
		...
		const shouldIncludeEvent = (key: string, event: CalendarEvent): boolean => {
