	...
	suspend fun dispatch(method: String, arg: List<String>): String {
		when (method) {
			...
			"queryAppStoreSubscriptionOwnership" -> {
				...
				val result: MobilePaymentSubscriptionOwnership = this.facade.queryAppStoreSubscriptionOwnership(
					customerIdBytes,
				)
				return json.encodeToString(result)
			}
			"queryPlaystorePaymentSubscriptionOwnership" -> {
				val customerIdBytes: DataWrapper? = json.decodeFromString(arg[0])
				val result: MobilePaymentSubscriptionOwnership = this.facade.queryPlaystorePaymentSubscriptionOwnership(
					customerIdBytes,
				)
				return json.encodeToString(result)
			}
